...
public class SalesforceBulkInputDialog extends BaseStepDialog implements StepDialogInterface {
	...
	private FormData fdInclModuleField,fdlInclModule,fdlInclSoapURL,fdInclSoapURL,fdlLimit, fdLimit, fdlMax, fdMax;
    ...
    private TextVar wTimeOut,wLimit,wMax;
	...
	public String open() {
		...
		// Row Max
		wlMax = new Label(wContentComp, SWT.RIGHT);
		wlMax.setText(BaseMessages.getString(PKG, "SalesforceBulkInputDialog.RowMax.Label"));
		props.setLook(wlMax);
		fdlMax = new FormData();
		fdlMax.left = new FormAttachment(0, 0);
		fdlMax.top = new FormAttachment(wTimeOut, 2*margin);
		fdlMax.right = new FormAttachment(middle, -margin);
		wlMax.setLayoutData(fdlMax);
		wMax = new TextVar(transMeta,wContentComp, SWT.SINGLE | SWT.LEFT | SWT.BORDER);
		props.setLook(wMax);
		wMax.addModifyListener(lsMod);
		fdMax = new FormData();
		fdMax.left = new FormAttachment(middle, 0);
		fdMax.top = new FormAttachment(wTimeOut, 2*margin);
		fdMax.right = new FormAttachment(100, 0);
		wMax.setLayoutData(fdMax);		
		...
		fdlUseCompression.top  = new FormAttachment(wMax, margin);
	...
	private void getInfo(SalesforceBulkInputMeta in) throws KettleException {
		...
		in.setRowMax(Const.NVL(wMax.getText(),"0"));
