...
public class SalesforceBulkConnection {
	...
	public void connect() throws KettleException{
		try {
	      ...
	      if ( getTimeOut() > 0 ) {
	        if ( log.isDebug() ) {
	          log.logDebug( BaseMessages.getString( PKG, "SalesforceBulkInput.Log.SettingTimeout", "" + this.timeout ) );
	        }
	      ...
	      if ( log.isDetailed() ) {
	        log.logDetailed( BaseMessages.getString( PKG, "SalesforceBulkInput.Log.LoginURL", config.getAuthEndpoint() ) );
	      }
	      ...
	      if ( log.isDetailed() ) {
	        ...
	        if ( getModule() != null ) {
	          log.logDetailed( BaseMessages.getString( PKG, "SalesforceBulkInput.Log.LoginModule", getModule() ) );
	        }
	        log.logDetailed( "<-----------------------------------------" );
	      }
		...
		/*try{
			...
			throw new KettleException(BaseMessages.getString(PKG, "SalesforceBulkInput.Error.Connection"), ce);
		} catch(Exception e){
	 ...
	 public void query() throws KettleException {
		...
		if(getBinding()==null)  throw new KettleException(BaseMessages.getString(PKG, "SalesforceBulkInput.Exception.CanNotGetBinding"));
	    ...
	    try {
			...
			if(!describeSObjectResult.isQueryable()) throw new KettleException(BaseMessages.getString(PKG, "SalesforceBulkInputDialog.ObjectNotQueryable",getModule()));
		    ...
		    for(int i=0; i<10000; i++) {
		    	Thread.sleep(i==0 ? 30 * 100 : 30 * 1000); //30 sec
